    ...
    public class QueryCRUD : Connection
    {
        ...
        public List<T_Po> GetT_Po()
        {
            try
            {
                ...
                string GetQuery = "select po.PoNo as poNo,po.Area as area, po.PoDate as PoDate , po.DelDate as DelDate , po.Inactive as Inactive,po.Total as Total ,po.SmtStatus as Smtstatus from ShowPO as po Order By po.SmtStatus";
                SqlCommand cmd = new SqlCommand(GetQuery, conn);
                SqlDataAdapter adapter = new SqlDataAdapter(cmd);
                DataTable dt = new DataTable();
        ...
        public bool EditDt(PoandPoApp poandPoApp)
        {
            try
            {
                connection();

                string QueryEdit = "update T_PoApp set SmtStatus = '2' where PoNo = '" + poandPoApp.PoNo + "'";
        ...
        public bool EditPR(PRandPRApp prandprapp)
        {
            try
            {
                connection();

                string QueryEdit = "update T_PR_App set SmtStatus = '2' where PR_no = '" + prandprapp.PR_no + "'";
